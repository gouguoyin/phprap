...
CREATE TABLE `doc_env` (
  ...
  KEY `name` (`name`),
  KEY `status` (`status`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='项目环境表';;
...
CREATE TABLE `doc_project_log` (
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COMMENT='项目日志表';
...
CREATE TABLE `doc_template` (
  ...
  `project_id` int(10) NOT NULL COMMENT '项目id',
  `header_fields` text NOT NULL COMMENT 'header参数，json格式',
  `request_fields` text NOT NULL COMMENT '请求参数，json格式',
